<script>
  ...
  function handleMouseMove(e) {
    ...
    const deltaY =
      ((e.clientY - dragStart.y) / rect.height) * 100 * sensitivity;

    // allow more freedom for dragging
    localOffsetX = dragStart.offsetX + deltaX;
    localOffsetY = dragStart.offsetY + deltaY;
  }
  ...
  function handleTouchMove(e) {
    ...
    const deltaX =
      ((e.touches[0].clientX - dragStart.x) / rect.width) * 100 * sensitivity;
    const deltaY =
      ((e.touches[0].clientY - dragStart.y) / rect.height) * 100 * sensitivity;
    ...
    >
      ...
      >
        <h2
          ...
          class="text-[14px] font-bold text-[var(--text-main)]"
        >
          Edit Image
        </h2>
        <button
      ...
      <div class="p-5">
        ...
        >
          ...
          <!-- Drag Hint Overlay -->
          <div
            class="absolute inset-0 flex items-center justify-center pointer-events-none opacity-0 hover:opacity-100 transition-opacity bg-black/20"
          >
            <div
              class="bg-black/60 text-white px-3 py-1.5 rounded-lg text-[11px] font-medium"
            >
              Drag to reposition
            </div>
          </div>
          ...
          <div class="absolute inset-0 pointer-events-none">
            <div
              class="absolute top-1/2 left-0 right-0 h-[1px] bg-white/20"
            ></div>
            <div
              class="absolute left-1/2 top-0 bottom-0 w-[1px] bg-white/20"
            ></div>
          </div>
        </div>
      ...
      <div class="px-5 pb-5 flex flex-col gap-4">
        ...
        <div class="flex flex-col gap-2">
          ...
          <div class="flex gap-2">
            {#each ["cover", "contain", "fill"] as fit}
              <button
                class="flex-1 h-9 rounded-lg text-[10px] font-bold uppercase tracking-[0.12em] transition-all border border-transparent"
                class:bg-[var(--accent)]={localFit === fit}
                class:text-[var(--app-bg)]={localFit === fit}
                class:bg-[var(--input-bg)]={localFit !== fit}
                class:text-[var(--text-muted)]={localFit !== fit}
                class:border-[var(--border-subtle)]={localFit !== fit}
        ...
        <div class="flex flex-col gap-2">
          ...
          <div class="grid grid-cols-2 gap-2">
            <div
              class="flex items-center gap-2 bg-[var(--input-bg)] rounded-lg px-3 py-2"
            >
      ...
      <!-- Footer -->
      <div
        class="flex items-center justify-end gap-3 px-5 py-4 border-t border-[var(--border-subtle)] bg-[var(--input-bg)]/50"
      >
        <button
        ...
        <button
          class="px-5 py-2.5 rounded-lg text-[11px] font-bold uppercase tracking-[0.12em] bg-[var(--accent)] text-[var(--app-bg)] hover:opacity-90 transition-all"
          onclick={handleSave}
        >
          Apply Changes
        </button>
